...
def send_error_log(log_text):
    ...
    try:
        ...
        if "403" in err_msg:
            print(f"{COLORS['ORANGE']}Пояснение: База Firebase отклонила запрос (Ошибка 403 Forbidden).")
            print(f"Скорее всего Google Cloud блокирует открытый REST API без API-ключа.{COLORS['RESET']}")
            
    # Если на сервер не улетело, сохраняем в локальный файл!
