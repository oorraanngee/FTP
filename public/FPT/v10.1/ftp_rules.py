...
def fix_print_heuristics(line):
    ...
    # ЗАЩИТА: Игнорируем закомментированные строки
...
def fix_keywords_heuristics(line):
    ...
    # Пропущено IN в цикле FOR (for i range(3): -> for i in range(3):)
    line = re.sub(r'^(\s*for\s+[a-zA-Z0-9_,\s]+?)\s+(?!in\b)([a-zA-Z_]\w*\s*\()', r'\1 in \2', line)
    
    # Забытая f перед строкой с {}
    line = re.sub(r'(?<![a-zA-Z])(["\'])(.*?(?:\{[a-zA-Z_][^{}]*\}).*?)\1', r'f\1\2\1', line)

    # Мусор от русской раскладки в конце строки
    ...
    # 4. Незакрытые скобки на одиночных строках (массивы, кортежи, словари)
