...
def update_dir_list():
    ...
    try:
        ...
        # Игнорируем скрытые файлы и кэш
...
def main():
    print(f"{COLORS['GREEN']}Программа FTP (Fix This Python) v8.0{COLORS['RESET']}")
    print("Команды: cd <папка>, check <файл/номер>, fix <файл/номер>, help, exit")
    ...
    while True:
        try:
            ...
            if cmd_input.lower() == 'exit': break
                ...
                print(f"{COLORS['ORANGE']}Совет: Вы можете использовать номер файла из списка вместо его имени!{COLORS['RESET']}")
                ...
                print(f"Неизвестная команда: '{cmd_input}'. Доступны: cd, check, fix, help, exit")
